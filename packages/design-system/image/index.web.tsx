...
import { ImageProps as ReactNativeImageProps, ImageURISource, ImageResizeMode } from 'react-native'
...
function Img({ source, loading = 'lazy', width, height, layout, resizeMode, ...props }: Props) {
	if (source?.uri && typeof source?.uri === 'string') {
		return (
			...
			<Image
				...
				blurDataURL={width > 40 && props.blurhash ? getBase64Blurhash(props.blurhash) : null}
				{...props}
			/>
		)
	}
	...
	// @ts-ignore
	return <Image src={source as string} loading={loading} width={width} height={height} {...props} />
}
