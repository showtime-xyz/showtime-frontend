...
import { StyleSheet, useWindowDimensions, LayoutChangeEvent } from 'react-native'
import Animated, {
	cancelAnimation,
	scrollTo,
	useAnimatedReaction,
...
const MaterialTabBar = <T extends TabName = any>({
	...
	useAnimatedReaction(
		...
		canSync => {
			if (canSync && scrollEnabled && itemsLayout.length === nTabs && itemsLayout[index.value]) {
				...
				if (offset < tabsOffset.value + halfTab || offset > tabsOffset.value + halfTab) {
	...
	return (
		<Animated.ScrollView
			...
			horizontal
			style={style}
			contentContainerStyle={[styles.contentContainer, !scrollEnabled && { width }, contentContainerStyle]}
