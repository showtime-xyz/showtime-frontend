...
type UseMotiAnimate<T> = T extends Animated.SharedValue<any> ? never : NonNullable<T>

const DropdownItemFocusRing = ({ isFocused }: { isFocused: Animated.SharedValue<boolean> }) => {
	// TODO moti should provide this
	const state = useDerivedValue<UseMotiAnimate<React.ComponentProps<typeof MotiView>['animate']>>(() => {
		return {
...
const useFocusedItem = ({ onFocus, onBlur }: { onFocus?: () => void; onBlur?: () => void }) => {
...
const DropdownMenuItem = DropdownMenu.menuify(
	({ tw, children, onBlur, onFocus, ...props }: { tw?: string } & ComponentProps<typeof DropdownMenu.Item>) => {
		...
		return (
			<DropdownMenu.Item {...props} onFocus={handleFocus} onBlur={handleBlur} style={tailwind.style(tw)}>
...
const DropdownMenuCheckboxItem = DropdownMenu.menuify(
	({
		...
		return (
			<DropdownMenu.CheckboxItem {...props} onFocus={handleFocus} onBlur={handleBlur} style={tailwind.style(tw)}>
				<DropdownItemFocusRing isFocused={isFocused} />
...
const DropdownMenuTriggerItem = DropdownMenu.menuify(
	({
		...
		return (
			<DropdownMenu.TriggerItem {...props} onFocus={handleFocus} onBlur={handleBlur} style={tailwind.style(tw)}>
				<DropdownItemFocusRing isFocused={isFocused} />
				{children}
