...
export default function ActivityCard({
  ...
  const handleUnfollow = async () => {
    ...
    context.setMyFollows(
      context.myFollows.filter((f) => f?.profile_id !== actor.profile_id)
    );
      ...
      <View tw="border-b border-gray-200 dark:border-gray-800 pb-4 px-4">
        <View tw="flex-row">
                  ...
                  <Image
                    source={{
                      uri: getProfileImageUrl(
                        actor.img_url || DEFAULT_PROFILE_PIC
                      ),
                    }}
          ...
          {context.user && (
            <View>
              <View
                onPress={onCornerMenuClick}
                tw="text-right text-gray-300 relative"
              >
