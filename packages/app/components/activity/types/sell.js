	...
	return (
		<View tw="flex-col">
			<View>
				{count === 1 && (
					<View tw="flex-row">
						<Text tw="text-gray-500 dark:text-gray-400">Sold </Text>
						<Link
						...
						<Text tw="text-gray-500 dark:text-gray-400"> to </Text>
						<Link href={`/${act.buyer?.username || act.buyer?.wallet_address}`}>
							<Pressable
								tw="hover:text-stpink dark:hover:text-stpink"
								onPress={() => mixpanel.track('Activity - Click on buyer name')}
							>
								<Text tw="text-black dark:text-gray-300">{act.buyer?.name}</Text>
							</Pressable>
						</Link>
