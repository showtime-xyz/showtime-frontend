...
const Footer = ({ isLoading }: { isLoading: boolean }) => {
	...
	if (isLoading) {
		return (
			<View tw="h-16 items-center justify-center mt-6 px-3" sx={{ marginBottom: tabBarHeight }}>
				<ActivityIndicator />
			</View>
		)
	}
...
const Feed = ({ activity, activityPage, getNext, isLoading, isRefreshing, onRefresh }: Props) => {
	...
	const renderItem = useCallback(({ item }) => <Card act={item} variant="activity" />, [])
	...
	const ListFooterComponent = useCallback(() => <Footer isLoading={isLoading} />, [isLoading])
