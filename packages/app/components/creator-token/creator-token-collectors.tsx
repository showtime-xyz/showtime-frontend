...
export const CreatorTokenCollectorsList = () => {
  const { height: screenHeight, width } = useWindowDimensions();
  const [creatorTokenId] = useParam("creatorTokenId");
  const isMdWidth = width >= breakpoints["md"];
  const { data: list, isLoading } = useCreatorTokenCollectors(creatorTokenId);
  const numColumns = 3;
  ...
  const ListHeaderComponent = useCallback(() => {
    return <CreatorTokensTitle title="Creator Token Collectors " />;
  }, []);
