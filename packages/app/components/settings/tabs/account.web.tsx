...
const WalletSocialAccounts = () => {
  ...
  const connected = useMemo(() => {
    let google = { subject: "" };
    let apple = { subject: "" };
    privyUser?.linkedAccounts?.forEach((v) => {
      if (v.type === "google_oauth") {
        google.subject = v.subject;
      }
      if (v.type === "apple_oauth") {
        apple.subject = v.subject;
      }
    });
...
type SocialConnectButtonProps = {
  connected: {
    google: { subject: string | null };
    apple: { subject: string | null };
  };
  type: "google" | "apple";
};
...
const SocialConnectButton = ({ connected, type }: SocialConnectButtonProps) => {
  ...
  return (
    ...
    >
      {accountSubject ? "Disconnect" : "Connect"}
    </Button>
  );
};
