...
class ERC20ForwarderClient {
  ...
  getApiId(req) {
    try {
      ...
      if (decoder) {
        ...
        const contractData =
          this.biconomyAttributes.dappAPIMap[req.to.toLowerCase()];
  ...
  async getTokenGasPrice(tokenAddress) {
    try {
      ...
      const tokenOracleDecimals =
        await this.oracleAggregator.getTokenOracleDecimals(tokenAddress);
  ...
  async buildTx({
    ...
    try {
      ...
      const tokenOracleDecimals =
        await this.oracleAggregator.getTokenOracleDecimals(token);
      ...
      let tokenContract = new ethers.Contract(token, tokenAbi, this.provider);
  ...
  async erc20ForwarderApproved(tokenAddress, userAddress, spendValue) {
    ...
    let token = new ethers.Contract(tokenAddress, tokenAbi, providerOrSigner);
