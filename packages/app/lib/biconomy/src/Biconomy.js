...
let ERC20ForwarderClient = require("./ERC20ForwarderClient").default;
let {
...
function Biconomy(provider, options) {
  ...
  this.signerFunction = options.signerFunction;
...
function getSignatureEIP712(engine, account, request) {
  ...
  let signerFunction = engine.signerFunction;

  if (!targetProvider && !signerFunction) {
  ...
  const promise = new Promise(async function (resolve, reject) {
    if (signerFunction) {
      try {
        let signature = await signerFunction(signTypedDataType, [
          account,
          dataToSign,
        ]);
