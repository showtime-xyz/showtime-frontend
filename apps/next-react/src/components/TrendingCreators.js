		...
		<>
							...
							<div
								className={`border rounded-full py-2 px-4 text-xs flex flex-row ${
									followAllClicked
										...
										: 'bg-stpurple text-white dark:text-gray-900 border-stpurple cursor-pointer hover:opacity-70'
								} transition-all`}
								onClick={
									context.user
										? followAllClicked
				...
				{isLoading ? (
					...
					shownLeaderboardItems.map((item, index) => (
						<LeaderboardItemV2
							...
							item={item}
							index={index}
							totalCount={shownLeaderboardItems.length}
						/>
					))
				)}
			</div>
			...
			{!isLoading && (
				<div className="flex flex-row items-center my-2 justify-center pb-10 sm:pb-0">
					{!showAllLeaderboardItems ? (
						<div
							className="bg-white dark:bg-gray-800 text-center px-6 py-2 mt-2 flex items-center w-max shadow-md rounded-full hover:text-stpink cursor-pointer"
							onClick={() => setShowAllLeaderboardItems(true)}
						>
