	...
	useEffect(() => {
		if (upPress) {
			...
			if (activeSelectedSearchResult === null)
				setActiveSelectedSearchResult(searchResults.length - 1 >= 0 ? searchResults.length - 1 : null)
			else
		...
		if (downPress) {
			...
			if (activeSelectedSearchResult === null)
				...
				setActiveSelectedSearchResult(
					activeSelectedSearchResult + 1 < searchResults.length ? activeSelectedSearchResult + 1 : null
				)
		}
		...
		<>
			...
			<div className="hidden flex-col relative w-full max-w-3xl md:flex pr-1">
				...
				{searchText.length > 0 && showSearchResults && (
					<div
						className="flex flex-col max-h-[80vh] overflow-y-scroll absolute top-10 inset-x-0 border dark:border-gray-800 bg-white dark:bg-gray-900 rounded-lg shadow"
						ref={dropdownRef}
					>
						{isLoading ? (
							...
							<>
									...
									>
										...
										>
											<img
												...
												src={
													searchResult?.img_url ? searchResult?.img_url : DEFAULT_PROFILE_PIC
												}
											/>
								...
								{searchResults.length === 0 && (
									<div className="flex w-full p-4 dark:text-gray-400">No matching people</div>
								)}
							</>
						)}
					</div>
			...
			{isMobileSearchOverlayOpen && (
				<div className="visible flex flex-col bg-transparent flex-1 z-1 py-2">
					<div className="flex items-center">
						<div className="flex relative w-full" ref={searchInputContainerRef}>
							<div
								className="flex absolute z-1 left-4 top-[12px] transform -translate-y-2.5 -translate-x-1 mr-3 text-black dark:text-gray-200 focus:-mt-px"
					...
					{searchText.length > 0 && showSearchResults && (
						<div
							className="flex flex-col mt-4 max-h-[80vh] overflow-y-scroll absolute top-10 sm:top-14 left-16 right-4 border dark:border-gray-800 bg-white dark:bg-gray-900 rounded-lg shadow"
							ref={dropdownRef}
						>
							{isLoading ? (
								<div className="flex justify-center w-full p-4">
								...
								<>
									{searchResults.map((searchResult, index) => (
										...
										>
											<div
												className={`flex items-center w-full p-4 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 ${
													activeSelectedSearchResult === index
														? 'bg-gray-800'
														: 'dark:bg-gray-900'
												} ${index === 0 ? 'rounded-t-lg' : ''} ${
											...
											>
												...
												/>
												<div className="truncate flex-0 dark:text-gray-300">
													{searchResult?.name || searchResult.address0}
												</div>
												{searchResult?.username ? (
													<div className="flex-1 text-sm pl-1 truncate text-gray-400 dark:text-gray-500">
														@{searchResult?.username}
													</div>
												) : null}
											</div>
