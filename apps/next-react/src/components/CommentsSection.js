...
export default function CommentsSection({
  ...
  const handleReply = (comment) => {
    ...
    if (comment.parent_id) {
      setParentComment(
        comments.find((com) => com.comment_id === comment.parent_id)
      );
        ...
        ? setCommentText(
            "@" +
              (siblingComment.username ||
                siblingComment.name ||
                `[${formatAddressShort(siblingComment.address)}](${
                  siblingComment.address
                }) `)
          )
        : setCommentText(
            "@" +
              (parentComment.username ||
                parentComment.name ||
                `[${formatAddressShort(parentComment.address)}](${
                  parentComment.address
                }) `)
          );
      ...
      >
        <Menu as={Fragment}>
          <div className="relative w-full">
            <MentionsInput
              value={
                (isReply && parentComment) || parentComment === null
                  ? commentText
                  : ""
              }
            ...
            >
              <Mention
                ...
                data={
                  parentComment ? handleSearchQuery : handleDebouncedSearchQuery
                }
          ...
          <Menu.Items className="absolute origin-top-left z-20">
            <Menu.Item>
              <Picker
                ...
                theme={resolvedTheme}
                onSelect={(emoji) =>
                  setCommentText((commentText) => commentText + emoji.native)
                }
              />
            </Menu.Item>
          </Menu.Items>
