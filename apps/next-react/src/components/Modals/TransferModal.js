...
const TransferModal = ({ open, onClose, token }) => {
	...
	return (
		<Transition.Root show={open} as={Fragment}>
			<Dialog
				static
				className="fixed xs:inset-0 overflow-y-auto z-1 pt-[96px] md:pt-0 modal-mobile-position w-full"
				open={open}
				onClose={trueOnClose}
			>
		...
		<>
			...
			<div className="p-4 flex items-center justify-between">
				...
				<Button
					...
					style="primary"
					disabled={quantity < 1 || !(addressRegex.test(address) || address.includes('.'))}
				>
					Transfer
				</Button>
			</div>
...
const SuccessState = ({ transactionHash }) => {
	return (
		<div className="p-12 space-y-8 flex-1 flex flex-col items-center justify-center">
			...
			<p className="font-medium text-gray-900 dark:text-white text-center">
				Your NFT has been transferred on the Polygon network. It might still show up on your profile for a few
				minutes.
			</p>
			<Button
				style="tertiary"
