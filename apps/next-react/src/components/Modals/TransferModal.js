...
const TransferModal = ({ open, onClose, token }) => {
  ...
  const trueOnClose = () => {
    if (isWeb3ModalActive.current || modalState === MODAL_STATES.PROCESSING)
      return;
  ...
  const transferToken = async () => {
    ...
    const { data } = await contract.populateTransaction.safeTransferFrom(
      signerAddress,
      address.includes(".") ? await web3.resolveName(address) : address,
      token.token_id,
      quantity,
      0
    );
    ...
    const transaction = await provider
      ...
      .catch((error) => {
        ...
        console.log(
          JSON.parse(error?.body || error?.error?.body || "{}")?.error?.message
        );
  ...
  return (
    <Transition.Root show={open} as={Fragment}>
      <Dialog
        static
        className="fixed xs:inset-0 overflow-y-auto z-[2] pt-[96px] md:pt-0 modal-mobile-position w-full"
        open={open}
        onClose={trueOnClose}
      >
...
const GeneralState = ({
  ...
  const { data: transferringTo, isValidating: loadingTransferAddress } = useSWR(
    () =>
      (addressRegex.test(address) || address.includes(".")) &&
      `/api/profile/card?wallet=${address}`,
    ...
    <>
      ...
      <div className="p-4 border-b border-gray-100 dark:border-gray-900 space-y-4">
        <div className="flex-1">
          ...
          <p className="text-sm font-medium text-gray-700 dark:text-gray-300">
            Paste an ENS domain or Ethereum address below
          </p>
        </div>
        {transferringTo?.profile_id ? (
          <div className="flex items-center justify-between rounded-3xl border-2 border-gray-100 dark:border-gray-900 p-4">
            <div className="flex items-center space-x-2">
              <img
                src={transferringTo.img_url || DEFAULT_PROFILE_PIC}
            ...
            <div>
              <button
                onClick={() => setAddress("")}
                className="p-2 bg-gray-100 dark:bg-gray-900 hover:bg-gray-200 dark:hover:bg-gray-800 rounded-full hover transition"
              >
                <XIcon className="w-4 h-4" />
              </button>
            </div>
          ...
          <input
            ...
            className="px-4 py-3 relative block rounded-2xl dark:text-gray-300 bg-gray-100 dark:bg-gray-900 border border-gray-300 dark:border-gray-700 focus:outline-none focus-visible:ring w-full"
            value={address}
...
const TransactionState = ({ transactionHash }) => {
  return (
    <div className="p-12 space-y-8 flex-1 flex flex-col items-center justify-center">
      ...
      <div className="space-y-1">
        <p className="font-medium text-gray-900 dark:text-white text-center">
          Your NFT is being transferred on the Polygon network.
        </p>
...
const SuccessState = ({ transactionHash }) => {
  return (
    <div className="p-12 space-y-8 flex-1 flex flex-col items-center justify-center">
      ...
      <p className="font-medium text-gray-900 dark:text-white text-center">
        Your NFT has been transferred on the Polygon network. It might still
        show up on your profile for a few minutes.
      </p>
      <Button
        style="tertiary"
        as="a"
...
const WalletErrorState = ({ transferToken }) => {
  return (
    ...
    >
      ...
      <p className="font-medium text-gray-900 dark:text-white text-center">
        The wallet you selected does not own this NFT.
      </p>
      <p className="font-medium text-gray-900 dark:text-white text-center max-w-xs mx-auto">
        Please{" "}
        <button
