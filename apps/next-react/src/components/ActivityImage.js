...
export default function ActivityImage({ nft, index, numberOfImages, openModal, spacingIndex, bottomRow, roundAllCorners, totalNumberOfImages, cardWidth }) {
	...
	return (
		...
		>
			{nft.mime_type && (
				<>
					{nft.mime_type?.startsWith('model') && showModel && <model-viewer src={nft.source_url} class="object-cover w-full h-full" autoplay camera-controls auto-rotate ar ar-modes="scene-viewer quick-look" interaction-prompt="none" onClick={event => event.stopPropagation()} />}
					{nft.mime_type?.startsWith('model') && !showModel && <img src={numberOfImages === 1 ? getImageUrlLarge(nft.still_preview_url ? nft.still_preview_url : nft.token_img_url, nft.token_aspect_ratio) : getImageUrl(nft.still_preview_url ? nft.still_preview_url : nft.token_img_url, nft.token_aspect_ratio)} className="object-cover w-full h-full" />}
					{nft.mime_type?.startsWith('image') && <img src={numberOfImages === 1 ? getImageUrlLarge(nft.still_preview_url ? nft.still_preview_url : nft.token_img_url, nft.token_aspect_ratio) : getImageUrl(nft.still_preview_url ? nft.still_preview_url : nft.token_img_url, nft.token_aspect_ratio)} className="object-cover w-full h-full" />}
			...
			{!nft.mime_type && (
				<>
					{nft.token_img_url && !((nft.token_has_video && !nft.mime_type?.startsWith('model')) || (nft.token_animation_url && !nft.token_img_url)) && <img src={numberOfImages === 1 ? getImageUrlLarge(nft.token_img_url, nft.token_aspect_ratio) : getImageUrl(nft.token_img_url, nft.token_aspect_ratio)} className="object-cover w-full h-full" />}
			...
			{nft.mime_type?.startsWith('model') && (
				<div
					className={`p-2.5 ${showModel ? '' : 'opacity-80 hover:opacity-100 cursor-pointer'} absolute top-1 right-1`}
					onClick={event => {
