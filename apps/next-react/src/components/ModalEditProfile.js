...
export default function Modal({ isOpen, setEditModalOpen }) {
  ...
  useEffect(() => {
    const getSocialLinkOptions = async () => {
      const optionsJson = await axios
        .get("/api/getsociallinkoptions")
        .then((res) => res.data);
          ...
          <form onSubmit={handleSubmit} className="p-4 overflow-y-auto">
            ...
            <div className="flex flex-col md:flex-row">
              <div className="flex-1 my-4 ">
                <div className="mb-3">
                  <p className="text-xl text-indigo-500 dark:text-indigo-400 mb-1">
                    Profile
                  </p>
                  {myProfile?.verified && (
                    <p className="text-gray-500 text-xs">
                      Verified users can't change their name or username. Please
                ...
                <div className="py-2">
                  <label
                    htmlFor="name"
                    className="block text-sm text-gray-700 dark:text-gray-500"
                  >
                    Name
                  </label>
                  <input
              ...
              <div className="flex-1 my-4 md:pr-4">
                <div>
                  <div className="text-xl text-indigo-500 dark:text-indigo-400 mb-3">
                    Links
                  </div>
