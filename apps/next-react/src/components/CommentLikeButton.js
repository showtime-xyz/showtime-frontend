...
const CommentLikeButton = ({ comment, openLikedByModal }) => {
  ...
  const handleCommentLike = async (comment_id) => {
    ...
    context.setMyCommentLikeCounts({
      ...
      [comment_id]:
        (context.myCommentLikeCounts &&
        !_.isNil(context.myCommentLikeCounts[comment?.comment_id])
          ? context.myCommentLikeCounts[comment?.comment_id]
    ...
    await axios
      ...
      .catch((err) => {
        if (err.response.data.code === 429) {
          context.setMyCommentLikes(
            context.myCommentLikes.filter((item) => !(item === comment_id))
          );
  ...
  const handleCommentUnlike = async (comment_id) => {
    ...
    // Post changes to the API
    await axios
      .post(`/api/unlikecomment/${comment_id}`)
      .then(() => mixpanel.track("Unliked comment"));
  };

  const like_count =
    context.myCommentLikeCounts?.[comment.comment_id] ?? comment.like_count;
      ...
      <div className="flex items-center">
        <button
          ...
          onClick={() =>
            context.user
              ...
              : handleLoggedOutCommentLike()
          }
        >
          <div
            className={`flex flex-row items-center rounded-md py-1 hover:text-stred ${
              context.disableLikes
                ? "hover:text-gray-500 text-gray-500"
                : "text-gray-500 dark:text-gray-600 dark:hover:text-stred"
            }`}
          >
        ...
        {like_count ? (
          <button
            onClick={() =>
              openLikedByModal(
                _.isEmpty(comment.likers) ? [context.myProfile] : comment.likers
              )
            }
          >
            <div className="ml-1 text-xs whitespace-nowrap text-gray-600 dark:text-gray-500 tabular-nums">
