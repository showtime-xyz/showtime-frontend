...
const CommentLikeButton = ({ comment, openLikedByModal }) => {
	...
	const handleCommentLike = async comment_id => {
		...
		context.setMyCommentLikeCounts({
			...
			[comment_id]:
				(context.myCommentLikeCounts && !_.isNil(context.myCommentLikeCounts[comment?.comment_id])
				...
				>
					...
					>
						<div className={`flex ${liked ? 'text-stred' : ''}`}>
							{liked ? <HeartIconSolid className="w-4 h-4" /> : <HeartIconOutline className="w-4 h-4" />}
						</div>
					</div>
				</button>
				{like_count ? (
					<button
						onClick={() =>
							openLikedByModal(_.isEmpty(comment.likers) ? [context.myProfile] : comment.likers)
						}
					>
