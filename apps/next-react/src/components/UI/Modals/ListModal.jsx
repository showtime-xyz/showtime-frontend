...
const ListModal = ({ open, onClose, onSuccess = () => null, token }) => {
  ...
  const listToken = async () => {
    ...
    if (
      !(await mintContract.isApprovedForAll(
        signerAddress,
        process.env.NEXT_PUBLIC_MARKETPLACE_CONTRACT
      ))
    ) {
    ...
    <>
      <div className="flex-1">
        ...
        <div className="p-4 border-b border-gray-100 dark:border-gray-900">
          ...
          <label
            className="text-sm font-medium text-gray-600 dark:text-gray-300 mt-1"
            htmlFor="price"
          >
          ...
          <div className="flex mt-4 space-x-2">
            ...
            <Dropdown
              ...
              options={Object.entries(LIST_CURRENCIES).map(
                ([ticker, address]) => ({
                  label: ticker,
                  value: address,
                })
          ...
          {hasPriceError ? (
            <p className="font-medium text-red-500 text-xs p-2 last:block">
              {priceErrorMessage}
            </p>
          ) : null}
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="space-y-1 text-gray-900 dark:text-white text-sm text-center font-medium leading-[1.4rem]">
        ...
        <p>
          You only need to do this once, and it shouldn't take more than a few
          seconds.
        </p>
      </div>
    </div>
