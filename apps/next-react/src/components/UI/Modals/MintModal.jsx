...
const MintModal = ({ open, onClose }) => {
	...
	useEffect(() => {
		...
		if (
			myProfile.wallet_addresses_excluding_email_v2?.filter(({ address }) => address.startsWith('0x'))?.length > 0
		)
			return
	...
	const saveDraft = () =>
		axios.post('/api/mint/draft', {
			...
			mime_type: sourcePreview.src ? `${sourcePreview.type}/${sourcePreview.ext}` : null,
	...
	const onFileUpload = async event => {
		...
		const type =
			file.type.split('/')[0] || (file.name.endsWith('.glb') || file.name.endsWith('.gltf') ? 'model' : '')
	...
	const isValid = useMemo(() => {
		...
		if (editionCount < 1 || editionCount > 10000 || royaltiesPercentage > 69 || royaltiesPercentage < 0)
			return false
	...
	<div>
		<div className="p-4 border-b border-gray-100 dark:border-gray-900">
			<p className="font-medium text-gray-900 dark:text-white">
				You’ll need to add an Ethereum wallet before creating an NFT on Showtime.
			</p>
		</div>
		<div className="p-4">
			<Link href="/wallet">
			...
			<div className="flex-1 overflow-y-auto">
					...
					<div className="flex items-center justify-between space-x-4">
						<div>
							...
							<p className="text-sm font-medium text-gray-700 dark:text-gray-300">
								{editionCount == 1 ? 'Unique' : editionCount.toLocaleString()} Edition
								{editionCount == 1 ? '' : 's'} / {royaltiesPercentage}% Royalties
							</p>
						</div>
				...
				<div className="p-4 border-b border-gray-100 dark:border-gray-900">
					<Checkbox value={hasAcceptedTerms} onChange={setHasAcceptedTerms}>
						I have the rights to publish this artwork, and understand it will be minted on the{' '}
						<span className="font-semibold text-gray-900 dark:text-white">Polygon</span> network.
					</Checkbox>
				</div>
			</div>
			<div className="p-4">
		...
		<div className="p-12 space-y-10 flex-1 flex flex-col items-center justify-center">
			...
			<div className="flex flex-col md:flex-row items-center space-y-2 md:space-y-0 md:space-x-4">
				<a
					...
					)}&text=${encodeURIComponent('🌟 Just minted an awesome new NFT on @tryShowtime!!\n')}`}
					target="_blank"
					rel="noreferrer"
				>
