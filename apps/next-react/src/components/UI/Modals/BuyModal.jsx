...
const BuyModal = ({ open, onClose, token }) => {
  ...
  const buyToken = async () => {
    ...
    const basePrice = parseBalance(
      token.listing.min_price.toString(),
      LIST_CURRENCIES[token.listing.currency]
    );
  ...
  const renderedPage = ((type) => {
    switch (type) {
      ...
      case MODAL_PAGES.NEEDS_ALLOWANCE:
        return (
          <AllowanceRequiredPage
            token={token}
            isWeb3ModalActive={isWeb3ModalActive}
  ...
  return (
    <Transition.Root
      show={open && modalVisibility}
      as={Fragment}
      afterLeave={afterModalCloseAnimation}
    >
      <Dialog
        static
            ...
            <div className="inline-block align-bottom rounded-t-3xl sm:rounded-b-3xl text-left overflow-hidden transform transition-all sm:align-middle bg-white dark:bg-black shadow-xl sm:max-w-lg w-full">
              <div className="p-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
                ...
                <button
                  ...
                  className="p-3 -my-3 hover:bg-gray-100 dark:hover:bg-gray-800 disabled:hidden rounded-full transition bg-gray-100"
          ...
          <>
            <div className="p-4 border-b border-gray-200 dark:border-gray-800 flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-600 dark:text-gray-400 font-bold text-sm">
                  {parseFloat(token.listing.min_price)} $
            ...
            <div className="p-4 border-b border-gray-200 dark:border-gray-800 flex items-center">
              <div className="flex items-center space-x-2">
                ...
                />
                <div>
                  <p className="font-medium text-gray-700 dark:text-gray-400 text-xs">
                    Created By
                  </p>
              ...
              <div className="flex items-center space-x-2">
                <img
                  alt={token.listing.name}
                  src={
                    token.listing.img_url
                      ? token.listing.img_url
                      : DEFAULT_PROFILE_PIC
                  }
                  className="rounded-full w-8 h-8"
            ...
            <div className="p-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
              <p className="text-gray-600 dark:text-white text-xs font-semibold">
                {token.listing.quantity}/{token.listing.total_edition_quantity}{" "}
                available
              </p>
              <p className="text-gray-600 dark:text-white text-xs font-semibold">
                {parseInt(token.listing.royalty_percentage)}% Royalties
              </p>
            </div>
            ...
            <div className="p-4 bg-gray-100 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
              ...
              <p className="text-gray-600 dark:text-gray-400 text-xs font-semibold">
                {token.listing.total_edition_quantity} Editions
              </p>
            </div>
          </>
        )}
...
const InvalidBalancePage = ({ buyToken, setModalPage, token }) => {
  return (
    ...
    >
      ...
      <p className="font-medium text-gray-900 dark:text-white text-center">
        You don't have enough ${token.listing.currency} on your wallet.
      </p>
      <p className="font-medium text-gray-900 dark:text-white text-center max-w-xs mx-auto">
        You can{" "}
        <button
  ...
  const grantAllowance = async () => {
    ...
    const tokenPermit = await signTokenPermit(
      ...
      await switchToChain(web3, 80001, {
        chainId: `0x${(process.env.NEXT_PUBLIC_CHAIN_ID == "mumbai"
          ? 80001
          : 137
        ).toString(16)}`,
        chainName:
    ...
    const transaction = await axios
      .post("/api/marketplace/permit", tokenPermit)
      .then((res) => res.data);
    ...
    <div className="flex flex-col min-h-[344px]">
      <div className="p-12 space-y-8 flex-1 flex flex-col items-center justify-center min-h-[344px]">
        ...
        <div className="space-y-1 text-gray-900 dark:text-white  text-sm text-center font-medium leading-[1.4rem]">
          ...
          <p>
            This shouldn't take more than a few seconds, but feel free to leave
