...
const BuyModal = ({ open, onClose, token }) => {
	...
	return (
		<Transition.Root show={open && modalVisibility} as={Fragment} afterLeave={afterModalCloseAnimation}>
			<Dialog
				static
				className="fixed xs:inset-0 overflow-y-auto z-1 pt-[96px] md:pt-0 w-full modal-mobile-position"
				open={open}
				onClose={updateModalVisibility}
			>
						...
						<div className="p-4 border-b border-gray-200 dark:border-gray-800 flex items-center">
							...
							<div className="flex items-center space-x-2">
								<img
									...
									src={token.listing.img_url ? token.listing.img_url : DEFAULT_PROFILE_PIC}
									className="rounded-full w-8 h-8"
								/>
								<div>
...
const AllowanceRequiredPage = ({ token, isWeb3ModalActive, setModalPage, setTransactionHash, buyToken }) => {
	...
	const grantAllowance = async () => {
		...
		const web3Modal = getWeb3Modal({ theme: resolvedTheme })
		isWeb3ModalActive.current = true
		const { web3, biconomy } = await getBiconomy(web3Modal, () => (isWeb3ModalActive.current = false)).catch(
			error => {
				if (error !== 'Modal closed by user') throw error
	...
	return (
		...
		>
			...
			<p className="font-medium text-gray-900 dark:text-white text-center">
				To buy this NFT, we need permission to spend your ${token.listing.currency}.
			</p>
			<div className="flex items-center justify-center">
		...
		<div className="flex flex-col min-h-[344px]">
			<div className="p-12 space-y-8 flex-1 flex flex-col items-center justify-center min-h-[344px]">
				...
				<div className="space-y-1 text-gray-900 dark:text-white  text-sm text-center font-medium leading-[1.4rem]">
					...
					<p>
						This shouldn't take more than a few seconds, but feel free to leave the page and come back
						later!
					</p>
				</div>
				<Button
