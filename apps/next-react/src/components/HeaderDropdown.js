...
const HeaderDropdown = () => {
	...
	useEffect(() => {
		...
		setHasEmailAddress(
			(myProfile?.wallet_addresses_v2 || []).length !==
					...
					<>
						<Menu.Button
							className={({ open }) =>
								...
								} p-1 -m-1 md:py-1 md:pl-2.5 md:pr-1 md:-my-1 md:-ml-2.5 md:-mr-1 flex items-center text-sm rounded-full focus:outline-none group transition`
							}
						>
							<span className="sr-only">Open user menu</span>
							<img
								className="h-8 w-8 rounded-full md:mr-2"
								src={getSmallImageUrl(myProfile?.img_url || DEFAULT_PROFILE_PIC)}
								alt={
									myProfile?.name ||
									myProfile?.username ||
									myProfile?.wallet_addresses_excluding_email_v2?.[0]?.ens_domain ||
									formatAddressShort(myProfile?.wallet_addresses_excluding_email_v2?.[0]?.address) ||
									'Profile'
								}
							/>
							<span className="hidden md:inline truncate dark:text-gray-200 transition font-semibold md:mr-2">
								{myProfile?.username
									? `@${myProfile.username}`
									: myProfile?.name ||
									  myProfile?.wallet_addresses_excluding_email_v2?.[0]?.ens_domain ||
									  formatAddressShort(
											myProfile?.wallet_addresses_excluding_email_v2?.[0]?.address
									  ) ||
									  'Profile'}
							</span>
						</Menu.Button>
						...
						>
							<Menu.Items
								static
								className="origin-top-right absolute right-0 mt-2 focus:outline-none w-full md:w-auto md:min-w-[15rem] px-4 md:px-0"
							>
								<Menu.Item>
										...
										<Menu.Item
											...
											className={({ active }) =>
												classNames(
													...
													'block rounded-lg w-full text-left py-2 pr-6 pl-2 text-sm font-medium transition'
												)
											}
											href={`/${
												myProfile?.username ||
												...
												{({ active }) => (
													<button
														...
														className={classNames(
															...
															'block rounded-lg w-full text-left pr-6 pl-2 py-2 text-sm font-semibold transition'
														)}
													>
														Add Email
													</button>
											...
											{({ active }) => (
												<button
													...
													className={classNames(
														...
														'block rounded-lg w-full text-left pr-6 pl-2 py-2 text-sm font-medium transition'
													)}
												>
													Sign Out
												</button>
