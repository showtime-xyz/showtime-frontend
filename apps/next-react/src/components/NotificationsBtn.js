...
export default function NotificationsBtn() {
	...
	useEffect(() => {
		...
		setHasUnreadNotifications(
			(notifs && notifs[0] && myProfile?.notifications_last_opened === null) ||
				(notifs &&
					notifs[0] &&
					new Date(notifs[0].to_timestamp) > new Date(myProfile?.notifications_last_opened))
		)
		...
		<Popover className="md:relative flex items-center justify-center">
			{({ open }) => (
				<>
					<Popover.Button
						data-close-notifs
						className={`dark:text-white transition rounded-full cursor-pointer relative h-6 w-6 box-content focus:outline-none ${
							open ? 'bg-gray-100 dark:bg-gray-900' : 'hover:bg-gray-100 dark:hover:bg-gray-900'
						} p-2 -m-2`}
					>
						<span onClick={open ? null : handlePanelOpen} className="flex items-center justify-center">
							<BellIcon className="w-6 h-6" />
							{hasUnreadNotifications && (
								<div className="bg-violet-500 absolute h-2 w-2 top-1.5 right-2.5 rounded-full" />
							)}
						</span>
					</Popover.Button>
					<Transition
					...
					>
						<Popover.Panel
							className={
								'text-black dark:text-gray-200 absolute text-center top-[3.1rem] md:top-10 right-0 z-20 w-full md:min-w-[25rem]'
							}
						>
							<div className="mx-2 md:mx-0 bg-white dark:bg-gray-900 py-2 px-2 shadow-lg rounded-xl transition border border-gray-200 dark:border-gray-800 overflow-y-scroll max-h-[80vh]">
								...
								{!isEmpty &&
									notifs.map(notif =>
										...
										notif.actors && notif.actors.length > 0 ? (
											...
											<div
												className={`py-3 px-3 hover:bg-gray-50 dark:hover:bg-gray-800 transition-all rounded-lg whitespace-nowrap flex items-start w-full max-w-full ${
											...
											>
												<div className="w-max mr-2 relative min-w-[2.25rem]">
													<Link
														href="/[profile]"
														as={`/${
															notif.actors[0]?.username || notif.actors[0].wallet_address
														}`}
													>
														<a onClick={closePanel}>
															<img
																alt={notif.name}
																src={
																	notif.img_url ? notif.img_url : DEFAULT_PROFILE_PIC
																}
																className="rounded-full mr-1 mt-1 w-9 h-9"
															/>
															<div
															...
															>
																<FontAwesomeIcon
																	...
																	icon={
																		iconObjects[
																			getNotificationInfo(notif.type_id).icon
																		]
																	}
																/>
															</div>
														...
														<>
															{notif.actors.length == 1 ? (
																...
																>
																	<a
																		onClick={closePanel}
																		className="text-black dark:text-gray-200 cursor-pointer hover:text-stpink dark:hover:text-stpink"
																	>
																		{notif.actors[0].name}{' '}
																	</a>
																</Link>
															...
															{notif.actors.length > 3 ? (
																<>
																	<ModalUserList
																		...
																		onRedirect={() => setOpenUserList(null)}
															...
															<span className="text-gray-500">
																{NOTIFICATION_TYPES.LIKED.includes(notif.type_id)
																	? 'liked '
																	: null}
																{NOTIFICATION_TYPES.FOLLOWED.includes(notif.type_id)
																	? 'followed you'
																	: null}
																{NOTIFICATION_TYPES.COMMENT.includes(notif.type_id)
																	? 'commented on '
																	: null}
																{NOTIFICATION_TYPES.COMMENT_MENTION.includes(
																	notif.type_id
																)
																	? 'mentioned you in '
																	: null}
																{NOTIFICATION_TYPES.COMMENT_LIKE.includes(notif.type_id)
																	? 'liked your comment on '
																	: null}
																{NOTIFICATION_TYPES.BOUGHT.includes(notif.type_id)
																	? 'bought '
																	: null}
															</span>
															{notif.nft__nftdisplay__name ? (
																<Link
																	href="/t/[...token]"
																	as={`/t/${Object.keys(CHAIN_IDENTIFIERS).find(
																		key =>
																			CHAIN_IDENTIFIERS[key] ==
																			notif.chain_identifier
																	)}/${notif.nft__contract__address}/${
																		notif.nft__token_identifier
																	}`}
																>
																	<a
																		onClick={closePanel}
												...
												as={
													getNotificationInfo(notif.type_id).goTo === 'profile'
														...
														: `/t/${Object.keys(CHAIN_IDENTIFIERS).find(
																key => CHAIN_IDENTIFIERS[key] == notif.chain_identifier
														  )}/${notif.nft__contract__address}/${
																notif.nft__token_identifier
														  }`
												}
											...
											>
												<div
													onClick={closePanel}
													className={`py-3 px-3 hover:bg-gray-50 dark:hover:bg-gray-800 transition-all rounded-lg cursor-pointer whitespace-nowrap flex items-start w-full max-w-full ${
														new Date(notif.to_timestamp) > new Date(previouslyLastOpened)
												...
												>
													<div className="w-max mr-2 relative min-w-[2.25rem]">
														<img
															alt={notif.name}
															src={notif.img_url ? notif.img_url : DEFAULT_PROFILE_PIC}
															className="rounded-full mr-1 mt-1 w-9 h-9"
														/>
														<div
															className="absolute bottom-0 right-0 rounded-full h-5 w-5 flex items-center justify-center shadow"
								...
								{isLoadingInitialData && (
									<div className="py-2 px-4 whitespace-nowrap">No notifications yet.</div>
								)}
								{!isReachingEnd && (
									<div className="flex justify-center items-center mb-1 mt-1">
										<button
											...
											className={`flex w-36 h-8 items-center justify-center  text-xs border-gray-600 text-gray-600 border rounded-full px-2 py-1 hover:text-stpink ${
												isLoadingMore ? '' : 'hover:border-stpink'
											} transition cursor-pointer`}
										>
											{isLoadingMore ? (
												<div className="inline-block w-6 h-6 border-2 border-gray-100 border-t-gray-800 rounded-full animate-spin"></div>
											) : (
												'Show More'
											)}
										</button>
