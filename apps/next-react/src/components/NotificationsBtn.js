...
export default function NotificationsBtn() {
  ...
  const closePanel = () =>
    document.querySelector("[data-close-notifs]").click();
  ...
  const updateNotificationsLastOpened = async () => {
    ...
    await setMyProfile(
      { ...myProfile, notifications_last_opened: new Date() },
      true
    );
  };
  ...
  const { data, error, size, setSize } = useSWRInfinite(
    (pageIndex, previousPageData) => {
      if (pageIndex != 0 && previousPageData.length < NOTIFICATIONS_PER_PAGE)
        return null;

      return `/api/notifications?page=${
        pageIndex + 1
  ...
  const isReachingEnd =
    isEmpty || (data && data[data.length - 1]?.length < NOTIFICATIONS_PER_PAGE);
      ...
      (notifs && notifs[0] && myProfile?.notifications_last_opened === null) ||
        (notifs &&
          ...
          new Date(notifs[0].to_timestamp) >
            new Date(myProfile?.notifications_last_opened))
    );
    ...
    <Popover className="md:relative flex items-center justify-center">
      {({ open }) => (
        <>
          ...
          >
            <span
              onClick={open ? null : handlePanelOpen}
                          ...
                          >
                            <a onClick={closePanel}>
                              ...
                              <div
                                ...
                                style={{
                                  backgroundColor: getNotificationInfo(
                                    notif.type_id
                                  ).color,
                                }}
                              >
                          ...
                          <div className="text-sm whitespace-pre-line">
                            <>
                              ...
                              <span className="text-gray-500">
                                {NOTIFICATION_TYPES.LIKED.includes(
                                  notif.type_id
                                )
                                  ? "liked "
                                  : null}
                                {NOTIFICATION_TYPES.FOLLOWED.includes(
                                  notif.type_id
                                )
                                  ? "followed you"
                                  : null}
                                {NOTIFICATION_TYPES.COMMENT.includes(
                                  notif.type_id
                                )
                                  ? "commented on "
                                  : null}
                                {NOTIFICATION_TYPES.COMMENT_MENTION.includes(
                                  ...
                                  ? "mentioned you in "
                                  : null}
                                {NOTIFICATION_TYPES.COMMENT_LIKE.includes(
                                  notif.type_id
                                )
                                  ? "liked your comment on "
                                  : null}
                                {NOTIFICATION_TYPES.BOUGHT.includes(
                                  notif.type_id
                                )
                                  ? "bought "
                                  : null}
