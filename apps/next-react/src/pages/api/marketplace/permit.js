...
export default handler()
	...
	.post(async ({ body }, res) => {
		...
		try {
			...
			res.status(errorMsg === 'Something went wrong.' ? 500 : 400).send(errorMsg)
		}
	})

const submitErc20Permit = (wallet, permit) => {
	const { v, r, s } = ethers.utils.splitSignature(permit.signature)
	const tokenContract = new ethers.Contract(permit.tokenAddr, ierc20PermitAbi, wallet)
...
const executeMetaTx = (wallet, metatx) => {
	...
	const tokenContract = new ethers.Contract(metatx.tokenAddr, ierc20MetaTxAbi, wallet)

	try {
		return tokenContract.executeMetaTransaction(metatx.owner, metatx.fnSig, r, s, v)
	} catch (error) {
		const revertMessage = JSON.parse(error?.error?.error?.body || '{}')?.error?.message
