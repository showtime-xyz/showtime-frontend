			...
			{typeof document !== 'undefined' ? (
				<>
					<ModalTokenDetail
						...
						item={itemOpenInModal?.nftGroup ? itemOpenInModal.nftGroup[itemOpenInModal?.index] : null}
						goToNext={goToNext}
						goToPrevious={goToPrevious}
						hasNext={!(itemOpenInModal?.index === itemOpenInModal?.nftGroup?.length - 1)}
						...
						<div className="grid md:grid-cols-3 xl:grid-cols-4">
								...
								<div className="px-4 py-4 h-max rounded-lg sticky top-24 bg-white dark:bg-gray-900 border border-transparent dark:border-gray-800 shadow-md">
									<div
										...
										className={`hover:bg-stblue100 mb-1 p-2 rounded-lg px-3 ${
											activityTypeFilter === 0 ? 'text-stblue bg-stblue100' : 'text-gray-500'
									...
									<div
										...
										className={`hover:bg-stred100 mb-1 p-2 rounded-lg px-3 ${
											activityTypeFilter === 1 ? 'text-stred bg-stred100' : 'text-gray-500'
										} hover:text-stred cursor-pointer transition-all flex flex-row items-center`}
									>
										{activityTypeFilter === 1 ? (
											<HeartSolidIcon className="w-5 h-5 mr-2.5" />
										) : (
											<HeartOutlineIcon className="w-5 h-5 mr-2.5" />
										)}
										<div>Likes</div>
									</div>
									<div
										...
										className={`hover:bg-stblue100 mb-1 p-2 rounded-lg px-3 ${
											activityTypeFilter === 2 ? 'text-stblue bg-stblue100' : 'text-gray-500'
										} hover:text-stblue cursor-pointer transition-all flex flex-row items-center`}
									>
										{activityTypeFilter === 2 ? (
											<CommentSolidIcon className="w-5 h-5 mr-2.5" />
										) : (
											<CommentOutlineIcon className="w-5 h-5 mr-2.5" />
										)}
										<div>Comments</div>
									</div>
									<div
										...
										className={`hover:bg-stgreen100 mb-1 p-2 rounded-lg px-3 ${
											activityTypeFilter === 4 ? 'text-stgreen700 bg-stgreen100' : 'text-gray-500'
										} hover:text-stgreen700 cursor-pointer transition-all flex flex-row items-center`}
									>
										{activityTypeFilter === 4 ? (
											<UserSolidIcon className="w-5 h-5 mr-2.5" />
										) : (
											<UserOutlineIcon className="w-5 h-5 mr-2.5" />
										)}
										<div>Follows</div>
									</div>
								</div>
							...
							<div className="col-span-2">
								...
								<InfiniteScroll
									...
									scrollThreshold={
										activityPage === 1 ? 0.2 : activityPage < 4 ? 0.3 : activityPage < 6 ? 0.7 : 0.8
									}
								>
									<ActivityFeed
										activity={activity}
