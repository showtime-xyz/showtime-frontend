...
const Wallet = () => {
	const { myProfile, user, setLoginModalOpen, setMyProfile, setMyLikes, setMyFollows, isMobile } =
	...
	const loginWithTezos = async () => {
		const { address: tezosAddr, publicKey: tezosPk } = await dAppClient.requestPermissions({
		...
		const signature = await dAppClient
			.requestSignPayload({ signingType: SigningType.MICHELINE, payload: payload })
													...
													{walletAddresses?.map(({ address, ens_domain }, i) => (
														...
														>
															...
															<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
																{address.startsWith('tz') ? 'Tezos' : 'Ethereum'}
															</td>
						...
						<div className="bg-white border border-transparent dark:border-gray-800 dark:bg-gray-900 shadow sm:rounded-lg">
							<div className="px-4 py-5 sm:p-6">
								...
								<div className="mt-2 max-w-xl text-sm text-gray-500">
									<p>
										Once you're signed in, you'll be able to add and remove your Ethereum and Tezos
										wallets.
									</p>
								</div>
								<div className="mt-5">
