const { withDangerousMod, WarningAggregator, AndroidConfig, withAppBuildGradle } = require('@expo/config-plugins')
...
async function editMainApplication(config, action) {
	const mainApplicationPath = AndroidConfig.Paths.getProjectFilePath(config.modRequest.projectRoot, 'MainApplication')
	...
	try {
		...
		WarningAggregator.addWarningAndroid('react-native-mmkv', `Couldn't modify MainApplication.java - ${e}.`)
	}
}
...
async function createMMKVJSIModulePackage(config, action) {
	...
	const filePath = path.join(packageRoot, `${packageName.split('.').join('/')}/MMKVJSIModulePackage.java`)

	try {
		return await saveFileAsync(filePath, getMMKVJSIModulePackage(packageName))
	} catch (e) {
		WarningAggregator.addWarningAndroid('react-native-mmkv', `Couldn't create MMKVJSIModulePackage.java - ${e}.`)
	}
}
