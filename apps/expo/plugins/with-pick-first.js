...
function addPickFirst(buildGradle, paths) {
	...
	if (packagingOptionsMatch) {
		console.warn('WARN: withPickFirst: Replacing packagingOptions in app build.gradle')
...
const withPickFirst = (config, props = {}) => {
	...
	return withAppBuildGradle(config, config => {
		if (config.modResults.language === 'groovy') {
			...
			throw new Error("withPickFirst: Can't add pickFirst(s) because app build.gradle is not groovy")
		}
		return config
	})
}

module.exports = (config, props) => withPlugins(config, [[withPickFirst, props]])
