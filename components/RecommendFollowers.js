...
const RecommendFollowers = ({ variant = RecommendFollowersVariants.ONBOARDING, items }) => {
    ...
    const filteredItems = showAllItems ? removeAlreadyFollowedItems : removeAlreadyFollowedItems.slice(0, 3);
    ...
    if (variant === RecommendFollowersVariants.ONBOARDING) {
        return recommendFollowersModalOpen && (
            <ClientOnlyPortal selector="#modal">
                <Backdrop onClick={() => closeModal()}>
                    ...
                    <Modal onClick={(e) => e.stopPropagation()}>
                        <Title>{context.isMobile ? "You might like" : "Creators you might like"}</Title>
                        <GraySeparator />
                        ...
                        {!showAllItems && removeAlreadyFollowedItems.length > 3 && (
                            <div
                                className="text-center px-6 py-2 mt-4 flex items-center w-max border-2 border-gray-300 rounded-full hover:text-stpink hover:border-stpink cursor-pointer"
                                onClick={() => {
