...
const TokenGridV4 = ({ items, isDetail, onFinish, filterTabs, isLoading }) => {
  ...
  const filteredItemsList = itemsList
    .filter((item) => {
  ...
  const goToNext = () => {
    const currentIndex = filteredItemsList.indexOf(currentlyOpenModal);
    if (currentIndex < filteredItemsList.length - 1) {
      ...
      if (filteredItemsList[currentIndex + 1].imageRef.current) {
        window.scrollTo({
          top:
            filteredItemsList[currentIndex + 1].imageRef.current.getBoundingClientRect()
              .top -
            bodyRect.top -
            70,
          behavior: "smooth",
      ...
      setCurrentlyOpenModal(filteredItemsList[currentIndex + 1]);
    }
  };
  ...
  useEffect(() => {
    ...
    const uniqueItems = Object.values(groupedItems)
      .map((itemGroup) => itemGroup.length > 1
  ...
  const currentIndex = filteredItemsList.findIndex(
      ...
      {typeof document !== "undefined" ? (
        <>
          <ModalTokenDetail
            ...
            hasNext={!(currentIndex === filteredItemsList.length - 1)}
