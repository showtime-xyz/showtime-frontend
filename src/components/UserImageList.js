		...
		<div className="flex -space-x-2 relative z-0 overflow-hidden p-0.5 -m-0.5">
			{users.map(user => (
				<Link href="/[profile]" as={`/${user?.username || user?.wallet_address}`} key={user.wallet_address}>
					<a className="relative z-10 inline-block h-12 w-12 rounded-full ring-2 ring-white dark:ring-gray-800 hover:opacity-90 transition overflow-hidden" onClick={() => mixpanel.track('Activity - Click on followed user')}>
