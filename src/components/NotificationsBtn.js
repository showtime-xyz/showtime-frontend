	...
	const handleMoreNotifications = async () => {
		if (!loadingMoreNotifications) {
			try {
				...
				const moreNotifs = await axios
					.get(`/api/notifications?page=${nextQueryPage}&limit=${NOTIFICATIONS_PER_PAGE}`)
					.then(res => res.data)
	...
	const getNotifications = async () => {
		try {
			const notifs = await axios
				.get(`/api/notifications?page=1&limit=${NOTIFICATIONS_PER_PAGE}`)
				.then(res => res.data)
			...
			setHasUnreadNotifications(
				(notifs && notifs[0] && context.myProfile.notifications_last_opened === null) ||
					(notifs &&
						notifs[0] &&
		...
		<div className="relative">
			...
			>
				...
				{hasUnreadNotifications && (
					<div className="bg-stpink absolute h-2.5 w-2.5 top-px left-px rounded-full" />
				)}
			</div>
			<div
				ref={dropdownRef}
				className={`overflow-y-scroll text-black absolute text-center top-10 right-0 bg-white py-2 px-2 shadow-lg rounded-xl transition-all transform border border-gray-200 z-1 ${
