		...
		<div className="p-2 my-1 flex rounded-xl hover:bg-gray-100 transition-all relative">
			<div className="mr-3 mt-1">
				<Link href="/[profile]" as={comment.username ? `/${comment.username}` : `/${comment.address}`}>
					<img alt={comment.username || comment.name || 'Unnamed'} src={comment.img_url ? comment.img_url : 'https://storage.googleapis.com/opensea-static/opensea-profile/4.png'} className="rounded-full cursor-pointer" style={{ height: 32, width: 32 }} onClick={closeModal} />
				</Link>
			</div>
			<div className="flex flex-col flex-1">
				<div className="flex items-center justify-between">
					<div className="flex flex-col sm:flex-row">
						<Link href="/[profile]" as={comment.username ? `/${comment.username}` : `/${comment.address}`}>
							<a className="hover:text-stpink cursor-pointer text-sm" onClick={closeModal} style={{ overflow: 'hidden', textOverflow: 'ellipsis' }}>
								{comment.name || formatAddressShort(comment.address)}
							</a>
						</Link>
						{comment.username && (
							<Link href="/[profile]" as={comment.username ? `/${comment.username}` : `/${comment.address}`}>
								<a className="hover:text-stpink cursor-pointer text-xs text-gray-400 sm:ml-1" onClick={closeModal} style={{ overflow: 'hidden', textOverflow: 'ellipsis' }}>
									@{comment.username}
								</a>
							</Link>
						)}
					...
					<div className={`text-gray-400 text-xs flex-0 sm:mb-0 ${comment.username ? '-mt-4' : '-mt-1'}  sm:mt-0`}>
					...
					{(isOwnerOfNFT || userWroteComment || isCreatorOfNFT) && (
						<div className={`flex items-center justify-center  relative ${comment.username ? '-mt-4' : '-mt-1'} sm:mt-0`}>
							<div onClick={toggleDropdown} className="ml-3 mr-1 cursor-pointer text-gray-400 hover:text-gray-600 transition-all">
								<FontAwesomeIcon
									style={{
										height: 14,
										width: 14,
							...
							<div ref={dropdownRef} className={`absolute text-center top-6 right-0 bg-white py-2 px-2 shadow-lg rounded-xl transition-all text-sm transform ${isActive ? 'visible opacity-1 translate-y-2' : 'invisible opacity-0'}`} style={{ zIndex: 1 }}>
								<div
				...
				<div className="text-gray-500 text-sm leading-5" style={{ wordBreak: 'break-word' }}>
					{commentWithMentions}
				</div>
			</div>
		</div>
