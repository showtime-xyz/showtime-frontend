			...
			<div className="flex flex-col flex-1">
				<div className="flex items-center justify-between">
					<div className="flex flex-col sm:flex-row">
						<Link href="/[profile]" as={comment.username ? `/${comment.username}` : `/${comment.address}`}>
							<a className="hover:text-stpink cursor-pointer text-sm truncate" onClick={closeModal}>
					...
					{(isOwnerOfNFT || userWroteComment || isCreatorOfNFT) && (
						<div className={`flex items-center justify-center  relative ${comment.username ? '-mt-4' : '-mt-1'} sm:mt-0`}>
							...
							<div ref={dropdownRef} className={`absolute text-center top-6 right-0 bg-white py-2 px-2 shadow-lg rounded-xl transition-all text-sm transform z-1 ${isActive ? 'visible opacity-1 translate-y-2' : 'invisible opacity-0'}`}>
								<div
