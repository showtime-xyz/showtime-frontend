...
const TokenGridV5 = ({
	...
	return (
		<>
			...
			<InfiniteScroll dataLength={dataLength} next={next} hasMore={hasMore} endMessage={endMessage} scrollThreshold={scrollThreshold}>
				{isLoading ? (
					...
					<>
						<div className={`grid ${extraColumn ? 'md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4' : 'lg:grid-cols-2 xl:grid-cols-3'}  overflow-hidden`}>
							{itemsList.map(item => (
								<TokenCard
									key={item.nft_id}
									originalItem={item}
