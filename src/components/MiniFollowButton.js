...
const MiniFollowButton = ({ profileId }) => {
	...
	return isFollowed === null ? null : !isFollowed ? (
		<button onClick={context.disableFollows ? null : context.user ? (isFollowed ? handleUnfollow : handleFollow) : handleLoggedOutFollow} className={`text-xs font-medium text-gray-800 rounded-xl px-3 py-2 transition ${context.disableFollows ? 'hover:opacity-80' : 'hover:bg-gray-100 focus-visible:bg-gray-100'}`}>
			Follow
		</button>
	) : null
}
