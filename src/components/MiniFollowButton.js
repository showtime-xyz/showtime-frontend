...
const MiniFollowButton = ({ profileId }) => {
	...
	return isFollowed === null ? null : !isFollowed ? (
		<div onClick={context.disableFollows ? null : context.user ? (isFollowed ? handleUnfollow : handleFollow) : handleLoggedOutFollow} style={{ marginTop: 1 }} className={`text-xs text-stlink opacity-80 cursor-pointer ${context.disableFollows ? ' hover:opacity-80' : ' hover:opacity-100'}`}>
			Follow
		</div>
	) : null
}
