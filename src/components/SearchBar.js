...
const handleSearchQuery = async (searchText, setSearchResults, setIsLoading) => {
	setIsLoading(true)
	try {
...
const handleDebouncedSearchQuery = AwesomeDebouncePromise(handleSearchQuery, 500)
	...
	useEffect(() => {
		if (upPress) {
			...
			if (activeSelectedSearchResult === null) {
				...
				setActiveSelectedSearchResult(activeSelectedSearchResult - 1 >= 0 ? activeSelectedSearchResult - 1 : null)
			}
		}
		if (downPress) {
			...
			if (activeSelectedSearchResult === null) {
				...
				setActiveSelectedSearchResult(activeSelectedSearchResult + 1 < searchResults.length ? activeSelectedSearchResult + 1 : null)
			}
		}
				...
				<SearchInputContainer ref={searchInputContainerRef}>
					...
					<SearchInput
						...
						onChange={e => {
							...
							handleDebouncedSearchQuery(e.currentTarget.value, setSearchResults, setIsLoading)
						}}
					/>
