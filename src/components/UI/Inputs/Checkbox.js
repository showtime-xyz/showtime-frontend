...
const Checkbox = ({ label, value, onChange, position = 'left', className }) => {
	return (
		<button type="button" onClick={() => onChange(!value)} className={`flex ${position == 'left' ? '' : 'flex-row-reverse'} items-center group ${className}`}>
			<div className="text-gray-800 dark:text-gray-200 border-gray-300 rounded bg-gray-100 dark:bg-gray-800 group-hover:bg-gray-200 dark:group-hover:bg-gray-700 focus-visible:bg-gray-200 dark:focus-visible:bg-gray-700 transition p-1">
