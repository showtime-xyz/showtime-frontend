...
const Dropdown = ({ options, value, onChange, label, disabled = false, className = '', inputClassName = '', optionClassName = '', optionInputClassName = '' }) => {
				...
				<>
					...
					<div className={`relative ${className}`}>
						...
						<Transition show={open} as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
							<Listbox.Options static className={`z-10 absolute mt-1 w-full border border-transparent dark:border-gray-800 bg-white dark:bg-gray-900 shadow-lg max-h-60 rounded-xl py-1 px-2 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm min-w-[8rem] ${optionClassName}`}>
								{options.map(item => (
									<Listbox.Option key={item.value} className={({ active }) => classNames(active ? 'text-gray-900 dark:text-gray-300 bg-gray-100 dark:bg-gray-800' : 'text-gray-900 dark:text-gray-400', `cursor-pointer select-none rounded-lg py-2 pl-3 pr-1 ${optionInputClassName}`)} value={item.value}>
