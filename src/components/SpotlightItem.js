...
class SpotlightItem extends React.Component {
	...
	async handleRefreshNFTMetadata() {
		...
		const result = await fetch(`/api/refreshmetadata/${this.state.thisItem.nft_id}`, {
			method: 'post',
		})
	...
	getImageUrl() {
		...
		if (img_url && img_url.includes('https://lh3.googleusercontent.com')) {
			this.state.thisItem.token_aspect_ratio && Number(this.state.thisItem.token_aspect_ratio) > this.aspect_ratio_cutoff ? (img_url = img_url.split('=')[0] + '=w2104') : (img_url = img_url.split('=')[0] + '=w1004')
		}
		return img_url
	}
			...
			<>
				...
				<CappedWidth>
					<div className="relative">
						<div ref={this.divRef} className="md:w-3/4 mx-auto flex items-center flex-col md:flex-row md:p-0">
							<div className="flex-1 text-right">
								<div>
									{item.token_has_video ? (
										<>
											<div className={`w-full h-full ${this.state.videoReady ? 'hidden' : null}`}>
												<div className="w-full text-center flex items-center mt-24 justify-center">
													<div className="loading-card-spinner" />
												</div>
											</div>
											<div className={`w-full shadow-lg h-full relative ${this.state.videoReady ? '' : 'invisible'}`}>
												<ReactPlayer
											...
											>
												{!this.state.imageLoaded ? (
													<div
														...
														style={{
															height: this.divRef?.current?.clientWidth ? this.divRef?.current?.clientWidth : 375,
														}}
													>
														<div className="loading-card-spinner" />
													</div>
													...
													style={{
														...(!this.state.imageLoaded
															...
															? {
																	...
																	height: item.token_aspect_ratio && this.divRef?.current?.clientWidth ? this.divRef?.current?.clientWidth / item.token_aspect_ratio : null,
															  }
															: {
																	backgroundColor: this.getBackgroundColor(item),
																	maxHeight: 500,
							...
							<div className="flex-1 text-left mt-3 md:mt-4 md:mt-0 md:pl-12 w-full p-6 pb-0 md:p-0">
										...
										<div className="absolute top-0 right-0 sm:right-6">
											<div
												onClick={e => {
													...
													this.props.setOpenCardMenu(this.props.openCardMenu == item.nft_id + '_' + listId ? null : item.nft_id + '_' + listId)
												}}
											...
											{this.props.openCardMenu == item.nft_id + '_' + listId ? (
												<div className="flex justify-end relative z-10">
													<div className={`absolute text-center top-2 bg-white shadow-lg py-2 px-2 rounded-xl transition-all text-md transform  ${this.props.openCardMenu == item.nft_id + '_' + listId ? 'visible opacity-1 ' : 'invisible opacity-0'}`} style={{ border: '1px solid #f0f0f0' }}>
														<div className="py-2 px-3 hover:text-stpink hover:bg-gray-50  transition-all rounded-lg cursor-pointer whitespace-nowrap" onClick={this.props.removeSpotlightItem}>
															Remove Spotlight
														</div>
														<div className="py-2 px-3 hover:text-stpink hover:bg-gray-50  transition-all rounded-lg cursor-pointer whitespace-nowrap" onClick={this.handleRefreshNFTMetadata}>
															Refresh Metadata
														</div>
													</div>
												</div>
								...
								<div>
									...
									{item.token_description ? (
										...
										>
											<div>
												{item.token_description?.length > this.max_description_length && !this.state.moreShown ? (
													<>
														{this.truncateWithEllipses(removeTags(item.token_description), this.max_description_length)}{' '}
														<a
															onClick={() => {
									...
									<div
										className={`flex ${
											//
											item.multiple_owners && this.props.pageProfile.profile_id !== item.creator_id ? 'flex-col lg:flex-row  pb-6' : 'flex-row'
										} pt-4 mt-8 w-full`}
									>
												...
												<div className="flex-shrink">
													<Link href="/c/[collection]" as={`/c/${item.collection_slug}`}>
														<a className="flex flex-row items-center">
															...
															<div className="mx-2 hover:text-stpink">{this.truncateWithEllipses(item.collection_name + ' Collection', 25)} </div>
														</a>
													</Link>
												</div>
											</div>
												...
												<div className="flex-shrink">
													<Link href="/[profile]" as={`/${item?.creator_username || item.creator_address}`}>
														<a className="flex flex-row items-center">
															...
															<div className="ml-2 hover:text-stpink">{this.truncateWithEllipses(item.creator_name, 25)}</div>
														</a>
													</Link>
												</div>
											</div>
													...
													{item.multiple_owners ? (
														...
														<Link href="/[profile]" as={`/${item?.owner_username || item.owner_address}`}>
															<a className="flex flex-row items-center">
																...
																<div className="hover:text-stpink">{this.truncateWithEllipses(item.owner_name, 25)}</div>
															</a>
														</Link>
													) : null}
												</div>
