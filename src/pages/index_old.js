...
export default function Home() {
	...
	useEffect(() => {
		const getFeatured = async () => {
			...
			const response_featured = await backend.get(`/v2/featured?limit=150&days=${featuredDays}`)
	...
	return (
		<Layout>
			<Head>
				...
				<meta property="og:image" content="https://storage.googleapis.com/showtime-nft-thumbnails/home_og_card.jpg" />
				...
				<meta name="twitter:image" content="https://storage.googleapis.com/showtime-nft-thumbnails/home_twitter_card_2.jpg" />
			</Head>
				...
				<>
					<div style={context.isMobile ? null : { paddingTop: 20 }}>
						<div className="m-auto" style={{ width: gridWidth }}>
							<div className="flex flex-row ml-0 mr-0" style={context.isMobile ? { padding: '0px 16px', marginBottom: 20 } : { padding: '0px 12px', marginBottom: 16 }}>
...
/>
						...
						<div className="m-auto" style={{ width: gridWidth, minHeight: 700 }}>
							<TokenGridV4 items={spotlightItems.slice(0, context.columns * 2)} isLoading={isLoadingSpotlight} />
						</div>
					</div>

					<div className="text-center pb-10 pt-4" style={context.columns === 1 ? { backgroundColor: 'rgb(243, 244, 246)' } : null}>
...
<FontAwesomeIcon style={{ height: 18 }} icon={faArrowRight} />
			...
			{columns && (
				<div className="mx-auto pt-2" style={columns === 1 ? { borderTopWidth: 1 } : { width: columns * (375 + 20) }}>
					{FilterTabs}
				</div>
			)}
