...
const Wallet = () => {
	...
	const loginWithTezos = async () => {
		...
		const payload = Buffer.from(`${process.env.NEXT_PUBLIC_SIGNING_MESSAGE_ADD_TEZOS_WALLET} ${nonce}`, 'utf8').toString('hex')
		const signature = await dAppClient.requestSignPayload({ signingType: SigningType.MICHELINE, payload: payload }).then(res => res.signature)
